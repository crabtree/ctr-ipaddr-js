...
describe("IPv4", function () {
    ...
    describe("toString", function() {
        ...
        const params = [
            { ctor: ["192.168.0.1/24"], result: "192.168.0.1/255.255.255.0" }
            , { ctor: ["192.168.0.1/24"], format: 2, result: "11000000101010000000000000000001/11111111111111111111111100000000" }
            , { ctor: ["192.168.0.1/24"], format: 16, result: "c0a80001/ffffff00" }
            , { ctor: ["172.16.0.1/16"], result: "172.16.0.1/255.255.0.0" }
            , { ctor: ["172.16.0.1/16"], format: 2, result: "10101100000100000000000000000001/11111111111111110000000000000000" }
            , { ctor: ["172.16.0.1/16"], format: 16, result: "ac100001/ffff0000" }
            , { ctor: ["10.0.0.1/8"], result: "10.0.0.1/255.0.0.0" }            
            , { ctor: ["10.0.0.1/8"], format: 2, result: "1010000000000000000000000001/11111111000000000000000000000000" }            
            , { ctor: ["10.0.0.1/8"], format: 16, result: "a000001/ff000000" }            
        ];

        params.forEach(function(param) {
            it(`returns ${param.result} for ${param.ctor}`, function () {
                const ip = new IPv4(...param.ctor);

                expect(ip.toString(param.format)).to.equal(param.result);
            });
        });

    });

    describe("belongsToSubnet", function() {

        const params = [
            { ctor: ["192.168.0.1/24"], addr: "192.168.0.5", result: true }
            , { ctor: ["192.168.0.1/24"], addr: "192.168.1.15", result: false }
            , { ctor: ["192.168.0.1/24"], addr: "192.168.0.0", result: true }
            , { ctor: ["192.168.0.1/24"], addr: "192.168.0.255", result: true }
            , { ctor: ["192.168.0.1/24"], addr: "1.1.1.1", result: false }
            , { ctor: ["192.168.0.1/24"], addr: 3232235521, result: true }
        ];
